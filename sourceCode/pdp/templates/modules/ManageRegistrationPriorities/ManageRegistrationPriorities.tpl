...
<div>
	...
	<p class="sectioncontent">
            ...
            {foreach from=$registrationPriorities item=curPriority}
                ...
                <TD style="width:70%">{$curPriority.rpName}<input type="hidden" name="name_{$curPriority.rpId}" maxlength=100 style="width:98%" value="{$curPriority.rpName}"/></TD>
                <input type="hidden" name="parameters_{$curPriority.rpId}" maxlength=255 style="width:98%" value="{$curPriority.rpParameters}" />
                </TR>
            {/foreach}
            </TABLE>
        </p>
